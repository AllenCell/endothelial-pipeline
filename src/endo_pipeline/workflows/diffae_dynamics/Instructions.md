...
For each workflow, `--dynamics_config_name` is an optional command line input to specify the config in `src/configs/dynamics_config.yaml` to use when running the workflow. If this is not specified via command line, the workflows are run using the `default` config in `dynamics_config.yaml`. Another optional input is `--model_name`, which specifies which deep learning feature manifests to use for analysis as available in `src/endo_pipeline/configs/models`. If this option is not specified, it defaults to `diffae_04_10`, i.e., analysis of the dynamics of the features generated by the `diffae_04_10` model.
