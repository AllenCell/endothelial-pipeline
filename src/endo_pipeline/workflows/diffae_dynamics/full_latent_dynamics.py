...
def main(list_of_datasets: list[str] | None = None) -> None:
    ...
    if list_of_datasets is None:
        list_of_dataset_configs = manifest_io.load_dataset_configs_with_manifest(
            "diffae_manifest_fmsid"
        )
    else:
        list_of_dataset_configs = [
            load_single_dataset_config(ds_name) for ds_name in list_of_datasets
        ]
    ...
    for ds_config in list_of_dataset_configs:
        print(f"Processing dataset: {ds_config.name}")
        df_ds = preprocessing.get_manifest_for_dynamics_workflows(ds_config, pca=None)
        ...
        fig.suptitle(f"Dataset: {ds_config.name}", y=0.95, fontsize=25)
        viz_base.save_plot(fig, f"{fig_savedir}/{ds_config.name}_latent_mean")

        fig, _ = manifest_viz.plot_latent_component_histogram(feats)
        fig.suptitle(f"Dataset: {ds_config.name}", y=0.95, fontsize=25)
        viz_base.save_plot(fig, f"{fig_savedir}/{ds_config.name}_latent_histogram")
        ...
        fig.suptitle(f"Dataset: {ds_config.name}", y=0.95, fontsize=25)
        viz_base.save_plot(fig, f"{fig_savedir}/{ds_config.name}_PC_histogram")
