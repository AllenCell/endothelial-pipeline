def main():
    """
    Run all main figure workflows.

    To register a main figure workflow, add a #main-figure tag to the docstring.

    ## Example usage

    To run all main figures:

    ```bash
    uv run endopipe run-all-main-figures
    ```
    ...
    results = asyncio.run(
        run_all_workflows_with_tag(
            workflow_name="run-all-main-figures",
            select_tag="main-figure",
