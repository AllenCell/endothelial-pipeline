...
def main(
    model_manifest_name: str = DEFAULT_MODEL_MANIFEST_NAME, run_name: str = DEFAULT_MODEL_RUN_NAME
) -> None:
    """
    ...
    for dataset_name in smad1_datasets:
        ...
        # Precompute mappings for positions
        positions = df_dataset["position"].unique()
        zarr_path_map = {
            pos: str(get_zarr_location_for_position(dataset_config, position=pos).path)
            for pos in positions
        }
        z_start_map = {}
        z_end_map = {}
        for pos in positions:
            if pos in dataset_config.center_z_plane:
                z_start_map[pos] = dataset_config.center_z_plane[pos] - LOWER_Z_SLICE_OFFSET
                z_end_map[pos] = dataset_config.center_z_plane[pos] + UPPER_Z_SLICE_OFFSET
            else:
                raise ValueError(f"Center z plane not defined for P{pos} in {dataset_name}")

        # Fast vectorized assignment using map with dicts
        df_dataset[ColumnName.ZARR_PATH] = df_dataset["position"].map(zarr_path_map)
        df_dataset[CytoDLLoadDataKeys.Z_START] = df_dataset["position"].map(z_start_map)
        df_dataset[CytoDLLoadDataKeys.Z_END] = df_dataset["position"].map(z_end_map)
        df_dataset[CytoDLLoadDataKeys.Z_STEP] = 1
        ...
        df_dataset["shear_stress_2"] = (
            shear_stress_list[1] if len(shear_stress_list) > 1 else np.nan
        )

        durations = [
            condition.stop - condition.start for condition in dataset_config.flow_conditions
        ]
        df_dataset["duration_at_ss_1_hr"] = durations[0] * 5 / 60
        df_dataset["duration_at_ss_2_hr"] = (
            (durations[1] * 5 / 60) if len(durations) > 1 else np.nan
        )

        df_smad1_list.append(df_dataset)

        if DEMO_MODE:
            logger.info("Demo mode active - processing only first dataset.")
            break
    ...
    prediction_filename_suffix = f"{model_manifest_name}_{run_name}"
