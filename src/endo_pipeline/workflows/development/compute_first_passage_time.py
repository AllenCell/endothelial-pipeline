...
def main(
    ...
    from endo_pipeline.settings.examples import FPT_CORRELATION_FIG_EXAMPLES
    ...
    fpt_stats_df.to_parquet(out_dir / "first_passage_time_stats.parquet")
    parameter_sweep_df.to_parquet(out_dir / "first_passage_time_parameter_sweep.parquet")
    ...
    for metric_to_plot in ["mean", "median"]:
        ...
        for nm, grp_df in fpt_stats_df_no_nan.groupby(
            ...
            if dataset_name in FPT_CORRELATION_FIG_EXAMPLES and metric_to_plot == "mean":
                plot_first_passage_time_correlations(
                    dataset_name=dataset_name,
                    first_passage_time_stats_df=grp_df,
                    fixed_point_id=fp_idx,
