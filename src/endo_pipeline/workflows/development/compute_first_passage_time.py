    ...
    for dataset_name in dataset_names:
        for crop_pattern in ["grid", "tracked"]:
            ...
            if fixed_points_df.empty:
                logger.warning(
                    f"No fixed points found for dataset {dataset_name}, skipping dataset."
                )
                continue
                ...
                ivp_args_mp.append(
                    {
                        ...
                        # simulate for up to 10x the duration of the measured trajectory
                        "trajectory_duration": track_duration * 10,
            ...
            with ProcessPoolExecutor(max_workers=n_proc) as executor:
                results = list(
                    tqdm(
                        executor.map(
                            solve_ddff_from_trajectory_initial_condition_helper, ivp_args_mp
                        ),
