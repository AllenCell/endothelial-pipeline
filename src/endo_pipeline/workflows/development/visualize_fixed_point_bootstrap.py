...
def main(
    ...
    **Visualizations**
    ...
    square, indeterminate = yellow plus); error bars show the per-coordinate
    bootstrap CIs at the percentiles used during the bootstrap run
    (`FP_CI_LOWER_PERCENTILE` and `FP_CI_UPPER_PERCENTILE`).
    ...
    if RESCALE_THETA:
        bounds_for_plots[ColumnName.DiffAEData.POLAR_ANGLE] = BIN_LIMITS_THETA_RESCALED

    # 2D projection pairs: (col_x_idx, col_y_idx, x_label, y_label)
    projections: list[tuple[int, int, str, str]] = [
        (0, 1, str(column_names[0]), str(column_names[1])),
        (0, 2, str(column_names[0]), str(column_names[2])),
    ]
    ...
    for dataset_name in dataset_names:
        ...
        n_total = len(df)
        high_confidence_df = df[df["bootstrap_detection_rate"] >= bootstrap_threshold].copy()
        ...
        high_confidence_df[ColumnName.DATASET] = dataset_name
        ...
        n_bootstrap = int(high_confidence_df["n_bootstrap_samples"].iloc[0])
        fig.suptitle(
            f"{dataset_name} — Bootstrap-Validated Fixed Points\n"
            f"(detection rate \u2265 {bootstrap_threshold:.0%}, "
            f"n bootstrap = {n_bootstrap})",
            fontsize=9,
        )

        for ax, (xi, yi, xlabel, ylabel) in zip(axes, projections, strict=False):
            col_x = column_names[xi]
            col_y = column_names[yi]
            ci_x_lo = f"{col_x}_ci_lower"
            ci_x_hi = f"{col_x}_ci_upper"
            ci_y_lo = f"{col_y}_ci_lower"
            ci_y_hi = f"{col_y}_ci_upper"

            for _, row in high_confidence_df.iterrows():
                stability = row.get(STABILITY_COLUMN_NAME, StabilityLabel.INDETERMINATE)
                color = STABILITY_COLOR_DICT.get(stability, "gray")
                marker = STABILITY_MARKER_DICT.get(stability, "o")

                x, y = float(row[col_x]), float(row[col_y])

                x_lo = float(row.get(ci_x_lo, np.nan))
                x_hi = float(row.get(ci_x_hi, np.nan))
                y_lo = float(row.get(ci_y_lo, np.nan))
                y_hi = float(row.get(ci_y_hi, np.nan))
            ...
            ax.set_xlim(bounds_for_plots[col_x])
            ax.set_ylim(bounds_for_plots[col_y])
    ...
    dataset_list = list(combined_df[ColumnName.DATASET].unique())

    fig_combined, axes_combined = plt.subplots(
        NROWS_2D_FLOW_FIELD, 1, figsize=FIGSIZE_2D_FLOW_FIELD
    )
    fig_combined.suptitle(
        f"Bootstrap-Validated Fixed Points — All Datasets\n"
        f"(detection rate \u2265 {bootstrap_threshold:.0%})",
        fontsize=9,
    )

    for ax, (xi, yi, xlabel, ylabel) in zip(axes_combined, projections, strict=False):
        col_x = column_names[xi]
        col_y = column_names[yi]
        ci_x_lo = f"{col_x}_ci_lower"
        ci_x_hi = f"{col_x}_ci_upper"
        ...
        for ds_name, ds_df in combined_df.groupby(ColumnName.DATASET):
    ...
    plt.tight_layout()
    save_plot_to_path(fig_combined, fig_savedir, "bootstrap_fixed_points_ci_combined")
    plt.close(fig_combined)
    logger.info("Saved combined bootstrap fixed point CI comparison figure to [ %s ].", fig_savedir)
