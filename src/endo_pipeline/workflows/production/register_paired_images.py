...
def main(
    ...
    if os.path.exists(save_path) and not DEMO_MODE:
        logger.error(
            f"Output directory {save_path.as_posix()} already exists. \
            Workflow exited to avoid overwriting. \
            To run this workflow, first delete directory {save_path.as_posix()}."
        )
    else:
        ...
        # Also, add a timestamp to the output directory name to avoid overwriting.
        if DEMO_MODE:

            # add timestamp to the path for saving outputs
        ...
        logger.debug(f"Save directory for aligned images set to: {save_path.as_posix()}.")
