...
def main(datasets: Datasets | None = None) -> None:
    ...
    the first position of the first dataset, using the first 100 timepoints.
    ...
    manifest_name = f"{TIMEPOINT_OUTLIERS_MANIFEST_NAME}{demo_suffix}"
    ...
    for dataset_name in dataset_names:
        ...
        if UPLOAD_TO_FMS:
            ...
            location.path = None
            logger.info(
                "Uploaded timepoint outlier annotations for dataset '%s' to FMS with id '%s'",
                dataset_name,
                fmsid,
            )
        ...
        tp_annotations = dataset_config.timepoint_annotations or {}
        for annotation, column in [
            (TimepointAnnotation.AUTO_BF_SCOPE_ERROR, Column.Annotations.AUTO_BF_SCOPE_ERROR),
