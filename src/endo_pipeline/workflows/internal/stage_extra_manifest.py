...
def main(
    ...
    """
    Stage files from selected extra manifest to S3 bucket.

    #internal #vast

    This workflow stages extras in the selected extra manifest to the internal
    ...
    ```bash
    uv run endopipe stage-extra-manifest MANIFEST_NAME -d
    ```

    To dry run the workflow:

    ```bash
    uv run endopipe stage-extra-manifest MANIFEST_NAME --dry-run
    ```

    To run the workflow live:

    ```bash
    uv run endopipe stage-extra-manifest MANIFEST_NAME --live-run
    ```

    To run the workflow for specific keys in the manifest:

    ```bash
    uv run endopipe stage-extra-manifest MANIFEST_NAME KEY_1 KEY_2
    ```
    ...
    ----------
    manifest_name
        Name of extra manifest containing extras to stage.
    datasets
    ...
    from endo_pipeline.manifests import load_extra_manifest
    from endo_pipeline.settings.manifest_staging import STAGING_EXTRA_MANIFEST_NAMES
    ...
    if manifest_name not in STAGING_EXTRA_MANIFEST_NAMES:
        logger.error("Manifest '%s' is not supported for staging to bucket", manifest_name)
        return

    # Load extra manifest and set staging folder equal to manifest name
    manifest = load_extra_manifest(manifest_name)
