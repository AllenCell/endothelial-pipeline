...
# %%
optical_flow_panel_size = (1.75 * 1.3, 1.75 * 1.3)
migration_coherence_panel_size = (1.5 * 1.3, 1.5 * 1.3)
...
# Width = MAX_FIGURE_WIDTH / 2; height matches so each subplot is square.
_half_width = MAX_FIGURE_WIDTH / 2.2
fig, axes = plt.subplots(
    2,
    2,
    figsize=(_half_width, _half_width),
    facecolor="white",
    squeeze=False,
)
fig.subplots_adjust(wspace=0.0, hspace=0.05, left=0, right=1, top=0.95, bottom=0.05)
...
fig_cb, ax_cb = plt.subplots(figsize=(MAX_FIGURE_WIDTH / 4, 0.1), layout="constrained")
