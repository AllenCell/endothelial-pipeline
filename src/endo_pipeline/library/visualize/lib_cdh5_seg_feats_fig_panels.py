...
def make_classic_feature_panels(datasets: list[str], out_dir: Path) -> None:
    ...
    for dataset_name in tqdm(datasets):
        ...
        feats_plot_args[ColNmSeg.ALIGNMENT_DEG]["label"] = "Cell Alignment (°)"
        feats_plot_args[ColNmSeg.NUCLEI_POSITION_ANGLE_DEG][
            "label"
        ] = "Cell-Nucleus Angle\nRel. to Flow (°)"
        feats_plot_args[ColNmSeg.CENTROID_VELOCITY_ANGLE_DEG]["label"] = "Migration Angle (°)"
        feats_plot_args[ColNmSeg.NUCLEI_POSITION_RELATIVE_MIGRATION_DEG][
            "label"
        ] = "Cell-Nucleus Angle\nRel. Migration (°)"
        feats_plot_args[ColNmSeg.NUCLEI_POSITION_RELATIVE_MIGRATION_DOTPROD][
            "label"
        ] = "Cell-Nucleus vs.\nMigration Dot Prod."
        ...
        for feat in feats_to_plot:
            ...
            if feat == ColNmSeg.NUCLEI_POSITION_RELATIVE_MIGRATION_DOTPROD:
