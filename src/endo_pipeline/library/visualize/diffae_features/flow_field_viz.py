...
def plot_flow_field_slices(
    ...
    if fig_savedir is not None:
        ...
        if df_cond is not None:
            ...
            condition = "from_data"
        viz_base.save_plot(
            fig, filename=fig_savedir / f"flow_field_{condition}_{dataset_name}", dpi=300
        )  # save the figure
        viz_base.save_plot(
            fig_,
            filename=fig_savedir / f"flow_field_streamplot_{condition}_{dataset_name}",
            dpi=300,
        )  # save the figure
...
def flow_field_viz_main(
    ...
    plt.show()
    # save the figure
    viz_base.save_plot(fig, fig_savedir / f"flow_field_{condition}_{name}_fp", dpi=300)
    ...
    plt.show()
    # save the figure
    viz_base.save_plot(fig, fig_savedir / f"flow_field_{condition}_{name}_traj", dpi=300)
    ...
    # save the figure
    viz_base.save_plot(
        fig, fig_savedir / f"flow_field_{condition}_{name}_traj_interpolated", dpi=300
    )
    return
