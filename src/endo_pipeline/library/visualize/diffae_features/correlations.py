...
def _add_exp_fit_to_plot(
    ...
    for i in range(3):
        ...
        if exp_decay_func == "exponential_decay":
            ...
            which_weight_is_larger = np.argmax(exp_fit[[0, 2]])
            logger.debug(
                "Full double exponential fit for PC%s: [ %.3f + %.3f exp(%.3f tau) + %.3f exp(%.3f tau) ]",
                i + 1,
                exp_fit[4],
                exp_fit[0],
                -exp_fit[1],
