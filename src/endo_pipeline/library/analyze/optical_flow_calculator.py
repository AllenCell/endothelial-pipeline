...
def expand_crop_region(crop_region: tuple[slice, ...], padding: int) -> tuple:
    ...
    crop_region = tuple(
        (
            ...
            crop.stop + padding if crop.stop else None,
        )
        for crop in crop_region
    )
...
class FlowCalculator:
    ...
    def get_features_from_vector_field_image(
        ...
        features_mags_mean, features_mags_std = (
            features_mags.mean(),
            features_mags.std(),
        )
        ...
        divergence = discrete_divergence_like(
            ...
            vector_field_image[:, chan_map["vy"], ...].squeeze(),
        )
        curl = discrete_divergence_like(
            vector_field_image[:, chan_map["vx"], ...].squeeze(),
            vector_field_image[:, chan_map["vy"], ...].squeeze(),
        )
        ...
        features = {
            "angle_of_mean_vector": angle_of_mean_vector,
            "magnitude_of_mean_vector": mag_of_mean_vector,
            "divergence_mean": features_diverg_mean,
            "divergence_std": feature_diverg_std,
            "curl_mean": features_curl_mean,
            "curl_std": features_curl_std,
            "vector_magnitudes_mean": features_mags_mean,
            "vector_magnitudes_std": features_mags_std,
            "vector_x_mean": features_vx_mean,
            "vector_x_std": features_vx_std,
            "vector_y_mean": features_vy_mean,
            "vector_y_std": features_vy_std,
        }
        return features
    ...
    def make_vector_field_map(
        ...
        ax.quiver(
            ...
            width=4,
            cmap=cmap,
            norm=plt.Normalize(*cmap_norm if cmap_norm else (n_.min(), n_.max())),
        )
        plt.tight_layout()
    ...
    def save_vector_field_as_pickle(self, out_dir: str | Path) -> None:
        ...
        vector_data = dict(
            zip(
                ("timeframe", "x", "y", "vx", "vy", "norm", "shape"),
                zip(
                    *[
                        (
                            tp,
...
def get_random_roi(
    ...
    """
    assert (
        isinstance(num_rois, int) and num_rois > 0
    ), f"num_rois must be a positive integer, not {num_rois}."
    assert len(image_shape) == len(
        roi_shape
    ), f"image_shape and roi_shape must have the same number of dimensions, but image_shape has {len(image_shape)} dimensions and roi_shape has {len(roi_shape)} dimensions."
...
def get_quadrant_means(
    points: np.ndarray, origin: tuple[float, float] = (0, 0)
) -> list[np.ndarray]:
...
def generate_synthetic_data() -> np.ndarray:
    ...
    for i in range(len(synth_img)):
        circle_centers = np.meshgrid(
            range(0, synth_shape_y, synth_shape_y // num_circles_per_axis),
        ...
        indices_too_high = np.any(
            circle_indices >= np.array([[synth_shape_y], [synth_shape_x]], ndmin=2),
            axis=0,
            keepdims=True,
        )
...
def compute_synthetic_image_flow_vectors_and_summarize(
    ...
    for i in range(0, len(synth_img) - 1, delta_t):
        ...
        flow = FlowCalculator.compute_flow(
            synth_img[i].squeeze(), synth_img[i + delta_t].squeeze(), radius=radius
        )
...
def generate_validation_plot(
    ...
    cmap = "summer"
    cmap_norm_all = np.asarray(
        [
            (
                vector_image[roi][:, vector_field_channel_map["norm"], ...].min(),
                vector_image[roi][:, vector_field_channel_map["norm"], ...].max(),
            )
            for roi in rois
        ]
    )
    ...
    ax2.scatter(
        features_and_pcs["vector_magnitudes_std"],
        features_and_pcs["divergence_std"],
        marker=".",
        c="grey",
        alpha=0.7,
    )
    ...
    for i, example_pt in example_points.items():
        roi = rois[i]
        quad_color, quad_record, crop_id = (
            example_pt["color"],
            example_pt["record"],
            example_pt["record"]["crop_id"].astype(int),
        )
        feats_and_pcs_at_roi = features_and_pcs.query("crop_id == @crop_id")
        dataset_name = feats_and_pcs_at_roi["dataset_name"].iloc[0]
        cropped_raw_img, cropped_vector_img = (
            raw_image[roi].compute(),
            vector_image[roi].compute(),
        )
        flow_graph_at_roi = get_trimmed_vector_field_map(
            cropped_raw_img.squeeze(),
        ...
        with plt.rc_context(
            ...
            ax4.arrow(
                x=float(quad_record["angle_of_mean_vector"]),
                y=0,
                dx=0,
                dy=0.9 * y_max,
                head_width=0.1,
                head_length=0.15 * y_max,
            ...
            [
                ...
                for theta in np.linspace(0, 2 * np.pi, 24 + 1)
            ]
    ...
    plt.tight_layout()
    crop_ids_for_filename = "-".join(
        [str(example_pt["record"]["crop_id"]) for example_pt in example_points.values()]
    )
...
def get_trimmed_vector_field_map(
    ...
    i, j = keep_me_indices
    keep_me_slices = (
        slice(np.min(i), np.max(i)),
        slice(np.min(j), np.max(j)),
        slice(None),
    )
...
def get_divergence_curl_example(
    ...
    return {
        "vector_field": example_vfields[vfield](show_vector_field),
