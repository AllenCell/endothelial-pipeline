...
def get_shear_stress_label_for_dataset(
    ...
    Label will be in format [ dataset name ] ([ shear stress value(s) ] dyn/cm^2).
    ...
    **Non-flow switch datasets**:
    ...
    (e.g. "12").
    """
    base_label_str = f"{dataset_config.name} (shear_stress dyn/cm{Unicode.SQUARED})"

    if len(dataset_config.shear_stress_regime) == 1:
        target_shear_stress = dataset_config.shear_stress_regime[0].target
        shear_stress_str = f"{target_shear_stress}"
    elif len(dataset_config.shear_stress_regime) == 2:
        if flow_condition is None:
            target_shear_stresses = [regime.target for regime in dataset_config.shear_stress_regime]
            shear_stress_str = "-".join(str(s) for s in target_shear_stresses)
        else:
