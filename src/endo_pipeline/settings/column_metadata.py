...
COLUMN_METADATA: dict[ColumnNameType, ColumnMetadata] = {
    ...
    Column.TRACK_LENGTH: ColumnMetadata(
        name="Track duration",
        description="Number of frames in track",
    ...
    Column.SegData.ORIENTATION: ColumnMetadata(
        ...
        unit="rad",
        description=f"Orientation of cell [radians] from 0 (along +x axis) to {Unicode.PI} (along -x axis)",
        min=0,
        max=pi,
        type=ColumnType.CONTINUOUS,
    ),
    Column.SegData.ORIENTATION_DEG: ColumnMetadata(
        ...
        label="Cell orientation",
        unit="°",
        description="Orientation of cell [degrees] from 0 (along +x axis) to 180 (along -x axis)",
        min=0,
        max=180,
        bin_width=5,
    ...
    Column.SegData.AREA_UM_SQ: ColumnMetadata(
        ...
        description=f"Area of cell segmentation [{Unicode.MU}m{Unicode.SQUARED}]",
        min=350,
        max=2000,
        type=ColumnType.CONTINUOUS,
    ),
    Column.SegData.ASPECT_RATIO: ColumnMetadata(
        ...
        description="Aspect ratio of ellipse fit to cell segmentation (major axis divided by minor axis)",
        min=1,
        max=10,
        type=ColumnType.CONTINUOUS,
    ),
    ...
    Column.SegData.CELL_FLUOR_MEAN: ColumnMetadata(
        ...
        label="Cell mean\nfluorescence",
        unit="a.u.",
        description="Mean of mEGFP-tagged VE-cadherin fluorescence [a.u.] in cytoplasmic region of cell segmentation",
        min=120,
        max=150,
        type=ColumnType.CONTINUOUS,
    ),
    ...
    Column.SegData.EDGE_FLUOR_MEAN: ColumnMetadata(
        ...
        unit="a.u.",
        description="Mean fluorescence [a.u.] at junctions between 2 cell segmentations (edge)",
        min=100,
        max=200,
        type=ColumnType.CONTINUOUS,
    ),
    ...
    Column.SegDataFilters.SMOOTHED_AREA_NORMD_DIFF: ColumnMetadata(
        ...
        description=f"Change in Gaussian-smoothed cell segmentation area [pixels{Unicode.SQUARED}]",
        type=ColumnType.CONTINUOUS,
    ),
    Column.SegDataFilters.NUM_VALID_TIMEPOINTS_IN_TRACK: ColumnMetadata(
        name="Number of valid timepoints",
        description="Number of timepoints in track that pass all segmentation-based filters",
        type=ColumnType.DISCRETE,
        unit="frames",
    ),
    ...
    Column.SegData.NUM_NUCLEI_IN_CROP: ColumnMetadata(
        ...
        description="Number of label-free predicted nuclei in a cell-centered patch",
        min=0,
        type=ColumnType.DISCRETE,
    ),
    # Dynamic features =========================================================
    Column.SegData.CENTROID_VELOCITY_ANGLE_DEG: ColumnMetadata(
        ...
        unit="°",
        description="Angle of cell migration based on change in centroid [degrees] with 0 along +x and 180/-180 along -x",
        min=-180,
        max=180,
        bin_width=5,
    ...
    Column.SegData.CENTROID_VELOCITY_UM_PER_MIN: ColumnMetadata(
        ...
        description=f"Speed of cell migration based on change in cell segmentation centroid [{Unicode.MU}m/minute]",
        min=0,
        max="max",
        type=ColumnType.CONTINUOUS,
    ),
    Column.SegData.NUCLEI_POSITION_RELATIVE_MIGRATION_DEG: ColumnMetadata(
        ...
        unit="°",
        description="Angle [degrees] of the nucleus centroid relative to cell segmentation centroid (counter-clockwise if positive, clockwise if negative, 0 if in the same direction as cell migration)",
        min=-180,
        max=180,
        bin_width=5,
        ticks=range(-180, 181, 90),
    ...
    Column.SegData.NUCLEI_POSITION_ANGLE_DEG: ColumnMetadata(
        ...
        unit="°",
        description="Angle of nucleus centroid from cell centroid relative to flow [degrees] with 0 along +x and 180/-180 along -x",
        min=-180,
        max=180,
        bin_width=5,
        ticks=range(-180, 181, 90),
    ...
    **{
    ...
    Column.OpticalFlow.UNIT_VECTOR_MEAN: ColumnMetadata(
        name="Patch-based migration coherence",
        label="Patch-based\nmigration coherence",
        description="Mean unit vector of optical flow vectors in a crop (EMA smoothing, alpha = 0.1)",
        min=0.3,
        max=0.9,
        bin_width=0.02,
    ...
    Column.OpticalFlow.SPEED_MEAN: ColumnMetadata(
        name="Patch-based migration speed",
        label="Patch-based\nmigration speed",
        unit="pixels/frame",
        description="Mean per-pixel speed [pixels/frame] of the optical flow vectors in a crop",
        min=0.8,
        max=4.9,
        ticks=[1, 2, 3, 4],
        type=ColumnType.CONTINUOUS,
