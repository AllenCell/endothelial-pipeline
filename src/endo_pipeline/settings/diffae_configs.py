"""Global settings for template DiffAE model configuration files."""

DIFFAE_MODEL_EVAL_CONFIG = "diffae_eval"
"""Name of the template configuration file for evaluating a DiffAE model."""

DIFFAE_MODEL_TRAIN_CONFIG = "diffae_train"
"""Name of the template configuration file for training a DiffAE model."""

DIFFAE_MODEL_FINETUNE_TRAIN_CONFIG = "diffae_finetune_train"
"""Name of the template configuration file for finetuning a DiffAE model."""

DIFFAE_MODEL_FINETUNE_EVAL_CONFIG = "diffae_finetune_eval"
"""Name of the template configuration file for evaluating a finetuned DiffAE model."""

DIFFAE_MODEL_LEGACY_CONFIG = "diffae_04_10_eval"
"""Name of the configuration file for evaluating the legacy DiffAE model diffae_04_10."""
