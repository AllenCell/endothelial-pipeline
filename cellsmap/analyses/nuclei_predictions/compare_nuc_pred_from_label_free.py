...
def get_image_data_from_original(dataset_name, scenes_to_use):
    ...
    for scene in scenes_to_use:
        ...
        img_dask_arr_nuc = img.get_image_dask_data(dim_order, C=[nuc_chan]).max(axis=dim_map[projection_dim], keepdims=True)
        img_dask_arr_bf_std = img.get_image_dask_data(dim_order, C=[bf_chan]).std(axis=dim_map[projection_dim], keepdims=True)
        img_dask_arr_bf = img.get_image_dask_data(dim_order, C=[bf_chan])
        bf_focus_index = np.argmin([img.std() for img in np.split(img_dask_arr_bf, img_dask_arr_bf.shape[dim_map[projection_dim]], axis=dim_map[projection_dim])])
        img_dask_arr_bf_near_focus = img.get_image_dask_data(dim_order, C=[bf_chan], Z=[max(bf_focus_index - 2, 0)])
        yield (scene_index,
               img_dask_arr_nuc,
               img_dask_arr_bf_std,
...
def get_image_data_from_zarr(dataset_name):
    ...
    for scene_index, filename in enumerate(img_dict_nuc):
        ...
        bf_focus_index = np.argmin([img.std() for img in np.split(img_dict_bf[filename], img_dict_bf[filename].shape[dim_map[projection_dim]], axis=dim_map[projection_dim])])
        Z_crop = [slice(max(bf_focus_index - 2, 0), max(bf_focus_index - 2, 0) + 1, None) if dim == dim_map[projection_dim] else slice(None) for dim in range(img_dict_bf[filename].ndim)]
...
for dataset_name in datasets_to_use:
    ...
    for (scene_index, img_dask_arr_nuc, img_dask_arr_bf_std, img_dask_arr_bf_near_focus) in imgs_to_eval:
        ...
        if use_original_data:
            cytodl_nuc_pred_path = [fp for fp in cytodl_nuc_pred_dir if dataset_name in str(fp.stem) and get_S_from_path(fp) == scene_index]
        else:
            cytodl_nuc_pred_path = [fp for fp in cytodl_nuc_pred_dir if dataset_name in str(fp.stem) and get_T_from_path(fp) == scene_index]
        assert len(cytodl_nuc_pred_path) == 1, f'Expected 1 file for {dataset_name} T{scene_index}, found {len(cytodl_nuc_pred_path)}'
        ...
        masks_bf_std = model_bf_stdproject.eval(img_arr_bf_std, channels=[0,0], min_size=50, flow_threshold=0.6, cellprob_threshold=0)

        overlay_bf = label2rgb(label=masks_bf_std[0], image=rescale_intensity(img_arr_nuc.squeeze()), bg_label=0)
        overlay_nuc = label2rgb(label=masks_bf_std[0], image=rescale_intensity(np.clip(img_arr_nuc.squeeze(), 0, np.percentile(img_arr_nuc.squeeze(), 98))), bg_label=0)
        plot_and_save_overlays(overlay_bf, overlay_nuc, out_dir, dataset_name, scene_index, filename_suffix='_cellpose')

        overlay_bf = label2rgb(label=cytodl_nuc_pred, image=rescale_intensity(img_arr_bf_near_focus), bg_label=0)
        overlay_nuc = label2rgb(label=cytodl_nuc_pred, image=rescale_intensity(np.clip(img_arr_nuc.squeeze(), 0, np.percentile(img_arr_nuc.squeeze(), 98))), bg_label=0)
        plot_and_save_overlays(overlay_bf, overlay_nuc, out_dir, dataset_name, scene_index, filename_suffix='_cytodl')

        overlay_bf = label2rgb(label=masks_bf_std[0].astype(bool)*1 + cytodl_nuc_pred.astype(bool)*2, image=rescale_intensity(img_arr_bf_near_focus), bg_label=0, colors=['red', 'cyan', 'yellow'])
        overlay_nuc = label2rgb(label=masks_bf_std[0].astype(bool)*1 + cytodl_nuc_pred.astype(bool)*2, image=rescale_intensity(np.clip(img_arr_nuc, 0, np.percentile(img_arr_nuc, 98))), bg_label=0, colors=['red', 'cyan', 'yellow'])
        plot_and_save_overlays(overlay_bf, overlay_nuc, out_dir, dataset_name, scene_index, filename_suffix='_cellpose_vs_cytodl')
        ...
        overlay_bf3 = label2rgb(label=ws, image=rescale_intensity(img_arr_bf_near_focus), bg_label=0)#, colors=['orange'])
        overlay_nuc3 = label2rgb(label=ws, image=rescale_intensity(np.clip(normd_nuc, 0, 0.1)), bg_label=0)#, colors=['orange'])
        plot_and_save_overlays(overlay_bf3, overlay_nuc3, out_dir, dataset_name, scene_index, filename_suffix='_classic')
...
fig, ax = plt.subplots()
sns.barplot(data=nuclei_count_df,
            x='dataset_name',
            y='nuclei_count',
            hue='method',
            ax=ax)
