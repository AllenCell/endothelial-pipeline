...
savedir = "//allen/aics/assay-dev/users/Erin/git-repos/cellsmap/cellsmap/analyses/20240917/diffae/"
...
# %%
p_hist_, bins_, centers_ = get_histogram_data(ndim,savedir,'high')
cdf = np.cumsum(p_hist_.ravel())
cdf = cdf / cdf[-1]
values = np.random.rand(1)
value_bins = np.searchsorted(cdf, values)
x_idx, y_idx = np.unravel_index(value_bins,
                                (len(centers_[0]), len(centers_[1])))
random_from_cdf = np.column_stack((centers_[0][x_idx], centers_[1][y_idx]))[0][:,None]
