...
def main(N_PROC: int=1, SHOW_PLOTS=True, SAVE_OUTPUT=True):
    ...
    vis.generate_alignment_summary_plots(df_ang_dist_summary, out_dir, SHOW_PLOTS, SAVE_OUTPUT)
    ...
    arg_list = [(dataset_name, time_hrs, T, grp, dist_min, dist_max, SHOW_PLOTS, SAVE_OUTPUT) for (dataset_name, time_hrs, T), grp in df_ang_dist.groupby(['dataset_name', 'Time (hours)', 'T'])]

    if N_PROC > 1:
        if __name__ == '__main__':
                with Pool(processes=N_PROC) as pool:
                    pool.starmap(vis.generate_alignment_plots, arg_list)
    else:
        for (dataset_name, time_hrs, T), grp in df_ang_dist.groupby(['dataset_name', 'Time (hours)', 'T']):
            vis.generate_alignment_plots(out_dir,
                                         dataset_name + f'_T{T}',
                                         time_hrs,
    ...
    plot_paths = sorted([filepath for filepath in Path.glob(out_dir / 'angles_vs_dists_polar', '*.tif')], key=lambda fp: preproc.extract_T(fp.stem, use_last_match=True))
