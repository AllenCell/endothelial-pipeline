...
class SteadyFP:
    ...
    def precompute_operator(self, f: np.ndarray, D: np.ndarray) -> None:
        ...
        if self.ndim == 1:
            ...
            self.A = np.einsum("i,ij->ij", -1j * self.k, f_hat[self.idx]) + np.einsum(
                "i,ij->ij", -self.k**2, D_hat[self.idx]
            )
        ...
        if self.ndim == 2:
            ...
            self.A = (
                ...
                * np.einsum(
                ...
                - np.einsum(
                    "i,ijkl->ijkl", self.k[0] ** 2, D_hat[0, self.idx[0], self.idx[1]]
                )
                - np.einsum(
                    "j,ijkl->ijkl", self.k[1] ** 2, D_hat[1, self.idx[0], self.idx[1]]
                )
            )
    ...
    def solve(self, f: np.ndarray, D: np.ndarray, verbose: bool = False) -> np.ndarray:
        ...
        if verbose:
            print(
                "%%%% Computing FP operator time: {0} seconds %%%%".format(
                    time() - start_fp_op
                )
            )
        ...
        q_hat = (
            tla.lstsq(
                torch.from_numpy(self.A[1:, 1:]).to(device),
                torch.from_numpy(-self.A[1:, 0]).to(device),
                rcond=1e-6,
            )[0]
            .cpu()
            .numpy()
