...
def compute_extrapolated_vector_field(kmcs:np.ndarray, 
    ...
    all_points = np.array(
        np.indices(vector_field[0].shape)
    ).reshape(len(vector_field[0].shape), -1).T
...
def get_callable_vector_field(vector_field_dict:dict,for_solve_ivp:bool=True) -> Callable:
    ...
    vec_field_grid = np.stack(vector_field_dict["vectors"], axis=-1) # shape (num_bins_x, num_bins_y, ... , ndim)
    xyz_grid = np.moveaxis(
        np.array(vector_field_dict["grid"]),0,-1
    ).reshape((-1,ndim))    
