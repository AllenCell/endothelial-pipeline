...
def plot_one_slice(velocities:Tuple, 
                   grid:Tuple, 
                   slice_indexes:np.ndarray, 
                   df_cond:pd.DataFrame,
                   ax:plt.Axes|None=None) -> Tuple[plt.Figure, plt.Axes]:
    """
    Plot one slice of the flow field.
    """
    if ax is None:
        _, ax = vb.init_subplots()

    ax.scatter(df_cond.PC1, df_cond.PC2, s=0.25, color="black", alpha=0.1)
...
def plot_flow_field_slices(flow_field_dict:dict, 
    ...
    ax1 = plot_one_slice((dU, dV), (xgrid, ygrid), zvalids, df_cond, ax=ax1)    
    ax2 = plot_one_slice((dU, dQ), (xgrid, zgrid), yvalids, df_cond, ax=ax2)
