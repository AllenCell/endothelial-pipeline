...
def main(config_name: str = "default") -> None:
    ...
    model_analysis.model_data_comparison(
        myModel, fig_savedir, pca, PCs, bins, ds_to_skip, pplane_xvec, pplane_yvec
    )
    ...
    model_analysis.run_fixed_point_analysis(
        f, shear_range_fpt, PCs, plt_lims, fig_savedir
    )

    ################### Entropy production rate as a function of shear stress ###################
    model_analysis.run_epr_analysis(
        myModel, bins, centers, shear_range_fpt, fig_savedir, additive_noise
    )
