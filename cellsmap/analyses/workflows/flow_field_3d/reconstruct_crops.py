...
for file_name in os.listdir(vtk_savedir):
    if "interpolated_mean_trajectory" in file_name:
        ...
        for i, origin in enumerate(
            [DDFF._bounds.xmin, DDFF._bounds.ymin, DDFF._bounds.zmin]
        ):
            coords[:, i] = DDFF.convert_coordinates_from_volume_to_pc(
                xvol=coords[:, i], origin=origin
            )
        ...
        tif_name = file_name.replace(
            "interpolated_mean_trajectory",
            "interpolated_mean_trajectory_reconstructed_crops",
        )
