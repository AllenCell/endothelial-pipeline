...
logfile="../logs/2d_bfcorr_lowFlow.txt"
...
# Subtract initial condition - correct for bias in data by patch position
...
np.save('../outputs/bins_bfcorr_lowFlow.npy',np.array(bins,dtype=object),allow_pickle=True)
np.save('../outputs/p_hist_bfcorr_lowFlow.npy',p_hist)

## KM average (coarse grained subsampling)
f_KM, a_KM, f_err, a_err = lg.KM_avg_2D(data, bins, stride=stride, dt=dt, multi_traj=True)
np.save('../outputs/KM_drift_bfcorr_lowFlow.npy',f_KM)
np.save('../outputs/KM_diff_bfcorr_lowFlow.npy',a_KM)
np.save('../outputs/KM_drift_err_bfcorr_lowFlow.npy',f_err)
np.save('../outputs/KM_diff_err_bfcorr_lowFlow.npy',a_err)
...
coeff_file = '../outputs/coeffs_bfcorr_lowFlow.npy'
cost_file = '../outputs/cost_bfcorr_lowFlow.npy'
