...
# %%
path_to_data = '//allen/aics/assay-dev/users/Benji/cellsmap/results/mase_cdh5_small_patch_norm/predictions.csv'
...
t_change = 283
dt = 5
X_t_high = X_t[:,:t_change,:]
num_traj = X_t_high.shape[0]
for i in range(num_traj):
    plt.plot(X_t_high[i,:,0],X_t_high[i,:,1],'k-',alpha=0.1)

# %%
# PDF of states
fig,ax = plt.subplots()
p_hist = np.load(savedir+'outputs/histogram.npy',allow_pickle=True)
bins = np.load(savedir+'outputs/histogram_bins.npy',allow_pickle=True)
bins = [bins[0].astype(float),bins[1].astype(float)]
...
Xi = np.load(savedir+'outputs/model_coeffs_high.npy')
V = np.load(savedir+'outputs/cost_vals_high.npy')
...
xvec = np.linspace(-20,25,50)
yvec = np.linspace(-10,25,50)
...
xvec = np.linspace(-10,30,100)
yvec = np.linspace(-15,25,100)
