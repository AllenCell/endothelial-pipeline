...
from torch.fft import fft, fftn, fftfreq, ifftn
...
class AdjFP:
    ...
    def derivs1d(x):
        ...
        Dx = sparse.spdiags(torch.vstack([diag2, diag1,diag0,-diag1.flip(0),-diag2.flip(0)]), torch.tensor([2,1,0,-1,-2]), shape=(N, N))
        ...
        Dxx = sparse.spdiags(torch.vstack([diag3, diag2, diag1, diag0, diag1.flip(0), diag2.flip(0), diag3.flip(0)]), torch.tensor([3,2,1,0,-1,-2,-3]), shape=(N, N))
    ...
    def derivs2d(x, y):
        ...
        Dy = sparse.spdiags(torch.vstack([diag2, diag1,diag0,-diag1.flip(0),-diag2.flip(0)]), torch.tensor([2,1,0,-1,-2]), shape=(Ny, Ny))
        # Repeat for each x-location
        Dy = torch.block_diag(*Dy.to_dense()[None,:,:].repeat(Nx,1,1))/(2*hy)
        Dy = Dy.to_sparse_csr()

        Dx = sparse.spdiags(torch.vstack([-torch.ones(Nx*Ny), torch.ones(Nx*Ny)]), torch.tensor([-Ny, Ny]), shape=(Nx*Ny, Nx*Ny)).to_dense()
    ...
    def operator2d(self, f, a):
        self.L = sparse.spdiags(f[0],torch.tensor([0]),(self.N[0]**2,self.N[0]**2)).to_sparse_csr().float() @ self.Dx  + \
              sparse.spdiags(f[1],torch.tensor([0]),(self.N[1]**2,self.N[1]**2)).to_sparse_csr().float() @ self.Dy + \
                 sparse.spdiags(a[0],torch.tensor([0]),(self.N[0]**2,self.N[0]**2)).to_sparse_csr().float() @ self.Dxx + \
                    sparse.spdiags(a[1],torch.tensor([0]),(self.N[1]**2,self.N[1]**2)).to_sparse_csr().float() @ self.Dyy
    ...
    def solve(self, tau, d=0):
        ...
        L_tau = tla.matrix_exp(self.L.to_dense()*tau)

        f_tau = torch.einsum('...ij,...ij->...i', L_tau, self.m1[d])/tau
        a_tau = torch.einsum('...ij,...ij->...i', L_tau, self.m2[d])/(2*tau)
...
class SteadyFP:
    ...
    def precompute_operator(self, f, a):
        ...
        if self.ndim == 2:
            ...
            f_hat = torch.zeros(np.append([self.ndim], self.N), dtype=np.complex64)
            a_hat = torch.zeros(f_hat.shape, dtype=np.complex64)
...
def ntrapz(I, dx):
    if isinstance(dx, int) or isinstance(dx, float) or len(dx)==1:
        return torch.trapz(I, dx=dx, axis=0)
    else:
        return torch.trapz( ntrapz(I, dx[1:]), dx=dx[0])
...
def cost2(Xi_flat, params):
    ...
    if Xi_flat.ndim == 1 and afp.ndim > 1:
        Xi = from_numpy(Xi_flat.reshape((len(params['f_expr'])+len(params['s_expr']),afp.ndim)))
    else:
        Xi = from_numpy(Xi_flat)

    # Kramers-Moyal coefficients (N[1] x N[2] x ... x N[ndim] x ndim arrays, reshaped to 2D)
    f_KM, a_KM = params['f_KM'].reshape((np.prod(N),2)), params['a_KM'].reshape((np.prod(N),2))
    ...
    mask = torch.where(torch.isfinite(f_KM))
    V = torch.sum(W[0, mask]*torch.abs(f_tau.T[mask] - f_KM[mask])**2) \
        + torch.sum(W[1, mask]*torch.abs(a_tau.T[mask] - a_KM[mask])**2)
...
def AFP_opt(cost, params):
    ...
    is_complex = torch.is_complex(Xi0[0])
    
    if is_complex:
        Xi0 = torch.cat((torch.real(Xi0), torch.imag(Xi0)))  # Split vector in two for complex
        opt_fun = lambda Xi: cost(Xi[:len(Xi)//2] + 1j*Xi[len(Xi)//2:], params)

    else:
        opt_fun = lambda Xi: cost(Xi, params)

    res = minimize(opt_fun, Xi0.flatten().numpy(), method='nelder-mead',
...
def SSR_loop(opt_fun, params):
    ...
    lib_f, lib_s = params['lib_f'].detach().clone(), params['lib_s'].detach().clone()
    Xi0 = params['Xi0'].detach().clone()
    
    m = len(f_expr) + len(s_expr)

    Xi = np.zeros((m, m-1), dtype=Xi0.numpy().dtype)  # Output results
