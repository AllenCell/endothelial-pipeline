...
for i in range(num_loc):
    X_t_low[i,:,:] = X_t_low[i,:,:] - X_t_high[i,0,:]
...
# Load outputs from `langevin_2D_bfcorr_lowFlow.py`
...
Xi_low = np.load('../outputs/coeffs_bfcorr_lowFlow.npy')
V_low = np.load('../outputs/cost_bfcorr_lowFlow.npy')
...
print("Cost at optimal sparsity: ", V_low[(2*ndim-1)-n_terms])
Xi_f_low = Xi_low[:len(f_expr), (2*ndim-1)-n_terms]
Xi_s_low = Xi_low[len(f_expr):, (2*ndim-1)-n_terms]
print("SINDy expression (drift): ")
print("     f_1(x1,x2) = ", np.round(Xi_f_low[:len(f_expr)//2],4).dot(f_expr[:len(f_expr)//2]))
print("     f_2(x1,x2) = ", np.round(Xi_f_low[len(f_expr)//2:],4).dot(f_expr[len(f_expr)//2:]))
print("SINDy expression (diffusion): ")
print("     sigma_1(x1,x2) = ", np.round(Xi_s_low[:len(s_expr)//2],4).dot(s_expr[:len(s_expr)//2]))
print("     sigma_2(x1,x2) = ", np.round(Xi_s_low[len(s_expr)//2:],4).dot(s_expr[len(s_expr)//2:]))
x1 = sympy.symbols('x1')
x2 = sympy.symbols('x2')
f1_low = sympy.lambdify([x1,x2], Xi_f_low[:len(f_expr)//2].dot(f_expr[:len(f_expr)//2]))
f2_low = sympy.lambdify([x1,x2], Xi_f_low[len(f_expr)//2:].dot(f_expr[len(f_expr)//2:]))

a1_low = sympy.lambdify([x1,x2], 0.5*(Xi_s_low[:len(s_expr)//2].dot(s_expr[:len(s_expr)//2]))**2)
a2_low = sympy.lambdify([x1,x2], 0.5*(Xi_s_low[len(s_expr)//2:].dot(s_expr[len(s_expr)//2:]))**2)

sigma1_low = sympy.lambdify([x1,x2], Xi_s_low[:len(s_expr)//2].dot(s_expr[:len(s_expr)//2]))
sigma2_low = sympy.lambdify([x1,x2], Xi_s_low[len(s_expr)//2:].dot(s_expr[len(s_expr)//2:]))
...
fig = pplane.plot_portrait(f1_low,f2_low,xvec,yvec)#,ICs=[inits[:,i] for i in range(len(inits_idx))],tVec=np.linspace(0,dt*t_change,100))

# %%
traj_model = stochastic_sim_EM(inits, lambda x: f_low(x[0],x[1]), lambda x: sigma_low(x[0],x[1]), t_change, dt)
for i in range(len(inits_idx)):
    plt.plot(traj_model[0,:,i],traj_model[1,:,i],'k-',alpha=0.5)
    plt.plot(X_t_low[inits_idx[i],:,0],X_t_low[inits_idx[i],:,1],'r-',alpha=0.25)
...
yvec = np.linspace(-30,15,100)
centers_new = [xvec,yvec]

X1,X2 = np.meshgrid(xvec,yvec)
f_vals_new = f_low(X1,X2)
a_vals_new = a_low(X1,X2)
...
# Solve time dependent Fokker-Planck
from my_fplanck import fokker_planck
# D1 = 0.5*Xi_s_low[:len(s_expr)//2][0]**2
# D2 = 0.5*Xi_s_low[len(s_expr)//2:][0]**2

# initialize time dependent fokker planck solver (only need for grid)
sim = fokker_planck(force=f_low,diffusion=a_low, extent=[[-25,10],[-20,15]],Ngrid=[35,35])

# %%
def kde_p0(X1,X2,indexing='ij'):
    data0 = X_t_low[:,5,:2].reshape(-1,2)
...
p0 = kde_p0(*sim.grid)
p0 = p0/np.sum(p0)
plt.imshow(p0.T,interpolation='nearest', origin='lower',
...
# %%
# my_mat = sim.master_matrix.toarray()
# for i, t in enumerate(time_vec):
#     if i == 0:
#         continue
#     if i % 20 == 0:
#         print('Time step {0}'.format(i))
#     Pt[i] = (expm(my_mat*t) @ p0.flatten()).reshape(sim.Ngrid)
# %%
# np.save('fokker_planck_lowFlow_sim.npy',Pt)

# %%
Pt = np.load('fokker_planck_lowFlow_sim.npy')
...
anim.save('corrected_lowFlow_fp.gif', writer=writer)

# %%
# Plot same sort of figure but for time evolution of histogram of low flow data
# def kde_p_any(data,X1,X2,indexing='ij'):
#     kde = KernelDensity(kernel="gaussian", bandwidth=1.75).fit(data)
#     if indexing == 'ij':
#         log_dens = kde.score_samples(np.array([X1.T.ravel(), X2.T.ravel()]).T)
#         N1 = X1.shape[0]
#         N2 = X1.shape[1]
...
# Pt_hist = np.zeros((Nsteps,)+tuple(sim.Ngrid))
# Pt_hist[0] = p0
# for i in range(len(time_vec)):
#     my_data = X_t_low[:,5+i,:2]
#     Pt_hist[i] = kde_p_any(my_data,*sim.grid)
# ### animation
# xlim = [sim.axes[0][1],10]
# ylim = [sim.axes[1][1],15]
# xvec = sim.grid[0]
# yvec = sim.grid[1]
...
#     global surf
#     surf.remove()
#     P_temp = np.where((xvec > xlim[1]) | (xvec < xlim[0]), None, Pt_hist[i])
#     P_temp = np.where((yvec > ylim[1]) | (yvec < ylim[0]), None, P_temp)
#     surf = ax1.plot_surface(xvec,yvec, P_temp, cmap='viridis')
#     ax1.set_xlim(xlim)
#     ax1.set_ylim(ylim)
...
# anim = FuncAnimation(fig, update_phist, frames=range(Nsteps), interval=30)
# plt.tight_layout()

# plt.show()
# HTML(anim.to_jshtml())
# writer = PillowWriter(fps=30,bitrate=1800)
# anim.save('corrected_lowFlow_phist.gif', writer=writer)
# # time_vec, Pt = sim.propagate_interval(kde_p0, tf, Nsteps=Nsteps)