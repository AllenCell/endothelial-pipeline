...
def initialize_workflow(dataset_name, SAVE_OUTPUT=True, IS_TEST=False):
    ...
    t_res = io.get_time_interval_in_minutes(dataset_name)
...
def run_workflow(dataset_name, SAVE_OUTPUT, IS_TEST, VERBOSE):
    ...
    if image_filepaths:
        ...
        raw_fps = io.get_dataset_info(dataset_name)['zarr_path']
        raw_channel = get_chan_map(raw_fps)[str(*[chan for chan in io.get_available_channels(dataset_name) if chan in ('CDH5', 'CDH5_Tubulin')])]
