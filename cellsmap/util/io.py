...
def localize_remote_filepath(filepath):

    if not Path(filepath).drive and isinstance(Path(filepath), WindowsPath):
        fp_localized = Path('\\'.join(Path(filepath).parts))

    else:
        fp_localized = Path(filepath)
