...
def convert_sldy_dataset(dataset: str, output_path: str, channel_names: list[str] = ["EGFP", "BF"]):
    ...
    # Path.mkdir(Path(output_path) / dataset, exist_ok=True, parents=True)
    ...
    for scene_index in range(len(img.scenes)):
        for position in range(n_positions):
            if n_positions > 1:
                ...
                # output = str(Path(output_path) / dataset / f"{dataset}_P{position}.ome.zarr")
            else:
                output = f"{output_path}/{dataset}/{dataset}_P{scene_index}.ome.zarr"
                # output = str(Path(output_path) / dataset / f"{dataset}_P{scene_index}.ome.zarr")
            print(f"Path: {output} exists={Path(output).exists()}")
