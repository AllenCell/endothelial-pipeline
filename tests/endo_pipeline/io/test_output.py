...
def mock_output_dir(tmp_path, mocker):
    output_dir_mock = mocker.patch("src.endo_pipeline.io.save.get_output_dir")
...
def mock_timestamp(mocker):
    ...
    datetime_mock = mocker.patch("src.endo_pipeline.io.save.datetime")
...
def test_get_output_path_file_name(mock_output_dir, mock_timestamp):
    ...
    assert path == mock_output_dir / mock_timestamp / "test_save"
...
def test_get_output_path_file_name_with_subdirs_no_timestamp(mock_output_dir):
    ...
    assert path == mock_output_dir / "test_save" / "subdir1" / "subdir2"
