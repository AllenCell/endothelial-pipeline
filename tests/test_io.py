...
def test_load_all_datasets():
    for ds in dataset_io.get_available_datasets():
        ...
        assert all(
            dim > 0 for dim in data.shape
        ), f"Dataset {ds} has invalid dimensions: {data.shape}"
...
def test_get_dataset_info():
    ...
    assert (
        ...
        == "//allen/aics/assay-dev/computational/data/holistic/endos/feasibility/20240305_T01_001.ome.zarr"
    )
...
def test_load_dataset():
    ...
    movie = dataset_io.load_dataset(
        "20240305_T01_001", channels=["CDH5_Tubulin"], time_end=2
    )
    assert movie.shape == (3, 1, 1712, 9592)
    # check start point specification
    movie = dataset_io.load_dataset(
        "20240305_T01_001", channels=["CDH5_Tubulin"], time_start=1, time_end=2
    )
    assert movie.shape == (2, 1, 1712, 9592)
    # check resolution specification
    movie = dataset_io.load_dataset(
        "20240305_T01_001", channels=["CDH5_Tubulin"], time_start=1, time_end=2, level=1
    )
    assert movie.shape == (2, 1, 856, 4796)
    movie = dataset_io.load_dataset(
        "20240305_T01_001",
        channels=["CDH5_Tubulin", "Nuc_Seg"],
        time_start=1,
        time_end=2,
        level=1,
    )
...
def test_get_model_info():
    ...
    assert (
        model_info["eval_config_path"]
        == "//allen/aics/assay-dev/users/Benji/cellsmap/cellsmap/model_features/configs/mae_cdh5/eval_config.yaml"
    )
